package com.Lvprasad.prod_ready_features_SpringBE.prod_ready_features_BE.clients.impl;


import com.Lvprasad.prod_ready_features_SpringBE.prod_ready_features_BE.advice.ApiResponse;
import com.Lvprasad.prod_ready_features_SpringBE.prod_ready_features_BE.clients.EmployeeClient;
...
public class EmployeeClientImpl implements EmployeeClient {
    ...
    public EmployeeDTO getEmployeeById(Long employeeId) {
        ...
        try {
            ResponseEntity<ApiResponse<EmployeeDTO>> employeeResponse = restClient.get()
                    .uri("employees/{employeeId}", employeeId)
                    .retrieve()
                    .onStatus(HttpStatusCode::is4xxClientError, (req, res) -> {
                        log.error("Status Code: {}", res.getStatusCode());
                        log.error("Response Body: {}", new String(res.getBody().readAllBytes()));
                        throw new ResourceNotFoundException("could not GET the employee");
                    })
                    .body(new ParameterizedTypeReference<>() {
                    });
//            assert employeeResponse != null;
//            assert employeeResponse.getBody() != null;
    ...
    public EmployeeDTO createNewEmployee(EmployeeDTO employeeDTO) {
        ...
        try {
            ...
            assert employeeDTOApiResponse.getBody() != null;
